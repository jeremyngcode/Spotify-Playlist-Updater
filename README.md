...
It's a little different for the cover image because a default image is still displayed in the absence of a custom provided one. What I've found is that the value for `['images'][0]['height']` in the [API response](https://developer.spotify.com/documentation/web-api/reference/get-playlist) is actually `None` if a custom cover image is provided, and a number if it's a default one. So I just check for truthiness. Unfortunately, I've also recently noticed this value being `None` for some smaller playlists (< 9 tracks I think?) without a custom cover image, so this litmus test isn't guaranteed to work for all cases as of now. But good thing the playlist image cover is probably the least important.
